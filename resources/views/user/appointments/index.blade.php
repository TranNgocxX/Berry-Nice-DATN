...
<div class="max-w-6xl mx-auto px-4 sm:px-6 py-8">
    ...
    <!-- Lọc & Tìm kiếm -->
    <div class="mb-8">
        <form id="filterForm" action="{{ route('appointments.index') }}" method="GET" class="flex flex-col md:flex-row gap-3">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div class="md:hidden divide-y divide-slate-100">
            @foreach($appointments as $appointment)
            <a href="{{ route('appointments.show', $appointment->id) }}" class="block p-5 active:bg-slate-50 transition-colors">
                <div class="flex justify-between items-start mb-3">
                    <div class="px-3 py-1 rounded-lg text-[10px] font-bold uppercase border {{ $statusClass }}">
                        {{ $appointment->status }}
                    </div>
                    <div class="text-xs text-slate-400">#{{ $appointment->id }}</div>
                </div>
                <h4 class="font-bold text-slate-800 text-lg mb-1">{{ $appointment->service->name }}</h4>
