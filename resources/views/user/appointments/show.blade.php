...
<div class="max-w-lg mx-auto px-4 py-8">
    <!-- Nút quay lại -->
    <div class="mb-4">
        <a href="{{ route('appointments.index') }}" class="inline-flex items-center text-[#6B8F71] hover:text-[#557A5E] transition group">
    ...
    <div class="bg-white rounded-2xl shadow-md border border-slate-200 overflow-hidden">
        @php
            $statusData = match($appointment->status) {
        ...
        <div class="text-center px-6 pt-6 pb-4 border-b border-slate-100">
            ...
            <h2 class="text-2xl font-bold text-slate-800 mt-2">#{{ $appointment->id }}</h2>
            <p class="text-sm text-slate-500">Đặt lúc {{ $appointment->created_at->format('H:i, d/m/Y') }}</p>
        </div>
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:justify-between gap-4 bg-slate-50 p-4 rounded-xl">
                <div>
                    ...
                    <p class="font-semibold text-slate-700">{{ $appointment->service->name }}</p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <p class="font-semibold text-slate-700">{{ $appointment->employee->name ?? 'Đang sắp xếp' }}</p>
            ...
            <div class="flex flex-wrap gap-2">
                <span class="px-3 py-1 bg-slate-100 rounded-lg text-xs font-bold text-slate-600">
                    {{ strtoupper($appointment->payment_method ?? 'cash') }}
                </span>
                <span class="px-3 py-1 rounded-lg text-xs font-bold {{ $appointment->payment_status == 'paid' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700' }}">
